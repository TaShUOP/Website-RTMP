...
3. **Frontend App**: The React application detects browser capabilities on load. If `flv.js` is supported, it decodes and plays the FLV stream with near-zero latency. If unsupported (e.g. iOS Safari), it instantly redirects to an isolated `/ios/` player that plays the HLS feed natively.
4. **Chat & Metrics**: Socket.io handles real-time chat messages and tracks active viewer counts across all connected clients (syncing seamlessly between the main app and the `/ios/` player).
...
2. **Frontend (`/frontend`)**: A React web application powered by Vite. It features a custom-designed, World Cup 2026 themed interface using **`flv.js`** for low-latency playback. It also contains an isolated `public/ios/index.html` file to provide an automatic, dedicated native HLS fallback for iPhone users.
